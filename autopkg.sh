...
Usage: $0: [options]
  ...
  --no-sign           Whether to sign packages (default true)
"
  exit 1
}
...
TEMP=$(getopt -o h --long help,validate,no-sign,push,packages:,upstream:,debian:,remote: -n 'autopkg.sh' -- "$@")
...
REPOAREA=$(mktemp -d --tmpdir=/tmp repo-area.XXX)
$PUSH || add_cleanup rm -r $REPOAREA
BUILDAREA=$(mktemp -d --tmpdir=/tmp build-area.XXX)
add_cleanup rm -r $BUILDAREA
REMOTE_URL=$(git config --get remote.$REMOTE.url)

echo "############################################################################"
echo "Will clone repo under $REPOAREA"
...
git clone $TOPLEVEL $REPOAREA

cd $REPOAREA
add_cleanup cd $TOPLEVEL

git remote add original_$REMOTE $REMOTE_URL
git checkout $LOCALUPSTREAM

add_checkpoint

# Create a temporary debian branch to do everything
TMPDEBIAN=$(mktemp -u debian.XXX)
git branch --track $TMPDEBIAN  origin/$LOCALDEBIAN
...
old_version=$(IFS="()" ; read  x v x < debian/changelog  ; echo $v)
snap=false
mrgextra=-m
dchextra="-R --new-version=$old_version"
...
git-buildpackage --git-export-dir="$BUILDAREA" \
                 --git-upstream-tree=$LOCALUPSTREAM \
                 --git-debian-branch=$TMPDEBIAN \
                 --git-export=INDEX \
                 --git-no-pristine-tar \
                 --source-option=--auto-commit \
                 --git-ignore-new ${SIGN:+"-uc -us"}

# do some dirty backup
# pkgarea might be needed by snf-deploy tool
rm -f "$PKGAREA"/* || true
...
if $VALIDATE; then
  check_remote original_$REMOTE $LOCALUPSTREAM
  check_remote original_$REMOTE $LOCALDEBIAN
fi
...
echo "##                  SUCCESS                    ##"
echo "#################################################"
echo "The cloned repo is under $REPOAREA ($TMPDEBIAN)"
echo "The packages are under $PKGAREA"
if $PUSH; then
  trap - EXIT
  echo "cd $REPOAREA"
  git remote -v
  echo "git push original_$REMOTE $(<$TAGFILE) $TMPDEBIAN:$LOCALDEBIAN"
  echo "git push original_$REMOTE $LOCALUPSTREAM:$LOCALUPSTREAM"
fi
