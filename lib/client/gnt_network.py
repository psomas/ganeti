...
def MapNetwork(opts, args):
  ...
  if group == "all":
    op = opcodes.OpNetworkConnectAll(network_name=network,
                                     network_mode=mode,
                                     network_link=link,
...
def UnmapNetwork(opts, args):
  ...
  if group == "all":
    op = opcodes.OpNetworkDisconnectAll(network_name=network,
                                        conflicts_check=opts.conflicts_check)
  else:
...
commands = {
  "add": (
    ...
    [DRY_RUN_OPT, NETWORK_OPT, GATEWAY_OPT, RESERVED_IPS_OPT, TAG_ADD_OPT,
  ...
  "modify": (
    ...
    [DRY_RUN_OPT, SUBMIT_OPT, RESERVED_IPS_OPT, GATEWAY_OPT,
     MAC_PREFIX_OPT, NETWORK_TYPE_OPT, NETWORK6_OPT, GATEWAY6_OPT],
